      ...
      if (isset($_FILES['pp']['name']) AND !empty($_FILES['pp']['name'])) {
         ...
         if($error === 0){
            ...
            if(in_array($img_ex_to_lc, $allowed_exs)){
               ...
               // update the Database
               $sql = "UPDATE users 
                       SET fname=?, username=?,email=?, city=?, pp=?
                       WHERE id=?";
               $stmt = $conn->prepare($sql);
               $stmt->execute([$fname, $uname,$email, $city, $new_img_name, $id]);
       	...
       	$sql = "UPDATE users
       	        SET fname=?, username=? , email=?, city=?
                WHERE id=?";
       	$stmt = $conn->prepare($sql);
       	$stmt->execute([$fname, $uname,$email,$city, $id]);
